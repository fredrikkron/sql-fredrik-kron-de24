...
-- wildcard selection/ SELECT */ SELECT ALL
...
FROM
	main.payment;
-- choose which columns not to return
SELECT
	* EXCLUDE(customer_id, amount)
FROM
	main.payment;

-- aggregation functions -> many-to-one mapping
SELECT 
...
CREATE VIEW IF NOT EXISTS main.payment_level AS (SELECT
	*,
	CASE
		WHEN amount < 3.5 THEN 'low cost'
		WHEN amount < 7.5 THEN 'medium cost'
